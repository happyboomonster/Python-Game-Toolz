...
class SFX_Manager(): # - A manager for keeping track of multiple Sound() objects at once and transmitting sound states over netcode -
    def __init__(self):
        self.sounds = [] #holds the pygame.mixer.Sound() objects of all the sounds we've loaded
    ...
    def enter_data(self, data, player_pos):
        ...
        for x in range(0,len(data[0])):
            ...
            if(ID_NUM > ID_NUM_MAX * 0.9): #almost at the wrap-around point?
                if(identification.ID > ID_NUM or identification.ID < ID_NUM_MAX * 0.1):
                    add = True
                    for x in self.sound_instances:
                        if(x.ID == identification.ID):
                            add = False
                            break
                    if(add):
